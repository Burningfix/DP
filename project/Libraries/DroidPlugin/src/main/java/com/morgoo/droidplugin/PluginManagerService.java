...
public class PluginManagerService /*extends Service*/ {
    ...
    public void onCreate() {
        //super.onCreate();
        keepAlive();
        getPluginPackageManager(mObj.getApplicationContext());
    }
    ...
    private void keepAlive() {
        try {
            ...
            mObj.startForeground(0, notification); // 设置为前台服务避免kill，Android4.3及以上需要设置id为0时通知栏才不显示该通知；
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        //这里要处理IntentService
        IActivityManagerHookHandle.getIntentSender.handlePendingIntent(mObj, intent);
